...
def obtainFilesRecursively(path, train_ratio):
    ...
    #config_file_pattern = r'h5_f_(\d{9})0\.h5'
    ...
    for dirname in dir_list_sim:
        ...
        for filename in os.listdir(os.path.join(path, dirname)):
            ...
            #file_number = int(config_file_match[1])
            #if file_number > 3000 and file_number % 10 != 0:
            #    continue
            # skip files finish
            #print(file_number)
...
def make_elu_grad_of(alpha):
    def grad(x):
        # `(N, in\_features)`
        assert(len(x.shape)==2)
        grad_batch = torch.where(x > 0.0, torch.ones_like(x), alpha * torch.exp(-torch.abs(x)))
